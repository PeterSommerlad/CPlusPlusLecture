...
void test_wcount_for_one_input_line() {
	std::istringstream input{"Asterix, Obelix und Mirculix marschieren durch den Wald."};
	ASSERT_EQUAL(8, wcount(input));
}

void test_wcount_for_multiple_input_lines() {
...
bool runAllTests(int argc, char const *argv[]) {
	...
	s.push_back(CUTE(test_wcount_for_one_input_line));
	s.push_back(CUTE(test_wcount_for_multiple_input_lines));
