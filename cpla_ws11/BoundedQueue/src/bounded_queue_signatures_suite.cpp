...
#if 0
void test_bounded_queue_front_type_is_reference_type() {
	BoundedQueue<int> queue{15};
	auto front_type = boost::typeindex::type_id_with_cvr<decltype(queue.front())>();
	auto expected_type = boost::typeindex::type_id_with_cvr<BoundedQueue<int>::reference>();
	ASSERT_EQUAL(expected_type.pretty_name(), front_type.pretty_name());
}

void test_const_bounded_queue_front_type_is_const_reference_type() {
	BoundedQueue<int> const queue{15};
	auto front_type = boost::typeindex::type_id_with_cvr<decltype(queue.front())>();
	auto expected_type = boost::typeindex::type_id_with_cvr<BoundedQueue<int>::const_reference>();
	ASSERT_EQUAL(expected_type.pretty_name(), front_type.pretty_name());
}

void test_bounded_queue_back_type_is_reference_type() {
	BoundedQueue<int> queue{15};
	auto back_type = boost::typeindex::type_id_with_cvr<decltype(queue.back())>();
	auto expected_type = boost::typeindex::type_id_with_cvr<BoundedQueue<int>::reference>();
	ASSERT_EQUAL(expected_type.pretty_name(), back_type.pretty_name());
}

void test_const_bounded_queue_back_type_is_reference_type() {
	BoundedQueue<int> const queue{15};
	auto back_type = boost::typeindex::type_id_with_cvr<decltype(queue.back())>();
	auto expected_type = boost::typeindex::type_id_with_cvr<BoundedQueue<int>::const_reference>();
	ASSERT_EQUAL(expected_type.pretty_name(), back_type.pretty_name());
...
#endif
void test_bounded_queue_pop_type_is_value_type() {
	BoundedQueue<int> queue{15};
...
cute::suite make_suite_bounded_queue_signatures_suite(){
	...
	s.push_back(CUTE(test_bounded_queue_pop_type_is_value_type));
