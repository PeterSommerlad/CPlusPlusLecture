...
auto const no_op_copy=[](char const *)->char *{return nullptr;};
}
...
struct value_holder{
	...
	template <typename T
	,typename =std::enable_if_t<(!std::is_same<std::decay_t<T>,value_holder>::value
			&& !std::is_array<std::remove_reference_t<T>>::value)>>
...
private:
	...
	using COPYTYPE=char *(*)(char const *);
	...
	static COPYTYPE make_copier(){
		return [](char const *s)->char *
				{
					if(s){
						char *p=new char[sizeof(std::decay_t<T>)];
						new (p) std::decay_t<T>{*reinterpret_cast<
							std::decay_t<T> const *>(s)};
						return p;
					}
					return nullptr;
				};
